...
for k,v in params.items():
    ...
    if ('Wlayer1' in k) or ('Woutput' in k):
        for i in range(v.shape[0]):
            for j in range(v.shape[1]):
                #   add epsilon
                v[i, j] = v[i, j] + eps
                #   run the network
                probs_p = forward(forward(x, params, 'layer1'), params, 'output', softmax)
                #   get the loss
                loss_p, _ = compute_loss_and_acc(y, probs_p)
                #   subtract 2*epsilon
                v[i, j] = v[i, j] - (2 * eps)
                #   run the network
                probs_m = forward(forward(x, params, 'layer1'), params, 'output', softmax)
                #   get the loss
                loss_m, _ = compute_loss_and_acc(y, probs_m)
                #   restore the original parameter value
                #   compute derivative with central diffs
